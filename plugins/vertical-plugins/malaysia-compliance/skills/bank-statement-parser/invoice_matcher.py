...
Matches parsed bank transactions with invoice JSON files
"""
...
class MatchResult:
    ...
    def __init__(self, status: str, confidence: float, warnings: List[str] = None):
        self.status = status  # matched, possible_match, unmatched, overpaid, underpaid
...
class InvoiceMatcher:
    ...
    def calculate_date_score(self, tx_date: date, invoice_date: date) -> float:
        """Calculate date matching score (0.0 to 1.0)"""
        if not invoice_date:
            return 0.5  # Neutral score if no invoice date
        ...
        if days_diff == 0:
            ...
            return 0.5
        elif days_diff <= self.date_tolerance.days:
            return 0.3
        else:
            return 0.0

    def calculate_amount_score(self, tx_amount: float, invoice_amount: float) -> float:
        """Calculate amount matching score (0.0 to 1.0)"""
        if invoice_amount == 0:
            return 0.0
        ...
        if difference <= tolerance:
            ...
            return 0.8
        elif difference <= invoice_amount * 0.05:  # 5% tolerance
            return 0.6
        elif difference <= invoice_amount * 0.10:  # 10% tolerance
            return 0.4
        else:
            return 0.0
    ...
    def match_transaction_to_invoice(self, transaction: Transaction, invoice: Dict) -> MatchResult:
        """Match a single transaction to a single invoice"""
        ...
        # Use absolute amount for matching (handle debit/credit direction)
        ...
        keyword_texts = [supplier_name, customer_name] + [item.get('description', '') for item in invoice.get('items', [])]
        ...
        if amount_score >= 0.8 and overall_score >= 0.7:
            if amount_difference_percent <= 0.02:  # Within 2%
    ...
    def match_statement_to_invoices(self, statement: BankStatement, invoice_paths: List[Path]) -> List[MatchResult]:
        """Match all transactions in a statement to available invoices"""
        results = []
        loaded_invoices = []
        ...
        # Match each transaction to the best invoice
    ...
    def generate_matching_report(self, results: List[MatchResult]) -> Dict:
        ...
        for result in results:
            ...
            # Add detail for each transaction
            detail = {
                "status": result.status,
                "confidence": round(result.confidence, 3),
                "transaction_date": result.matched_transaction.date.isoformat() if result.matched_transaction else None,
                "transaction_description": result.matched_transaction.description if result.matched_transaction else None,
